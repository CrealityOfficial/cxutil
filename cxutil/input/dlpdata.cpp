...
namespace cxutil
{
	...
	void DLPData::traitPolys(int layer, DLPPolys& polys) const
	{
		...
		if (layer >= 0 && layer < layers())
		{
			const cxutil::Polygons& polygons = impl->layersData.at(layer).polygons;
	...
	std::vector<std::vector<trimesh::vec2>> DLPData::traitPolys(int layer) const
	{
		...
		if (layer >= 0 && layer < layers())
		{
			const cxutil::Polygons& layerPolygons = impl->layersData.at(layer).polygons;
	...
	void DLPData::calculateVolumeAreas(SliceInfo& info) const
	{
		...
		{
			const cxutil::DLPLayer& cxLayer = impl->layersData.at(i);
			...
			cxutil::LightOffCircle circle;
			cxutil::lightOffDistance(cxLayer.polygons, circle);
